...
public class ListeCommande extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Commandecomplet prod=new Commandecomplet(); prod.setCommande(id); prod.filtre();
        prod.setAfficher(true);
        Vector vec=new Vector();
        vec=prod.selectV(connex.getConnexion());
        Commandecomplet[] liste=new Commandecomplet[vec.size()];
        vec.copyInto(liste);
        request.setAttribute("liste",liste);
        connex.getConnexion().close();

        RequestDispatcher dispat = context.getRequestDispatcher( "/listecommande.jsp" );
